...
#ifndef CONFIG_TFT_DISPLAY_CONTROLLER_RA8875
...
#define TPCR0_ADC_TIMING ((CONFIG_DISP_TOUCH_RA8875_SAMPLE_TIME << 4) | CONFIG_DISP_TOUCH_RA8875_ADC_CLOCK)
...
void ra8875_touch_init(void)
{
    ...
    GB_DEBUGI(DISP_TAG, "Initializing RA8875 Touch...");
...
{
    GB_DEBUGI(DISP_TAG, "%s touch.", enable ? "Enabling" : "Disabling");
...
#if DEBUG
        GB_DEBUGI(DISP_TAG, "Touch Poll Raw: %d,%d", x, y);
#endif
...
#if DEBUG
        GB_DEBUGI(DISP_TAG, "Touch Poll - Event: %d; %d,%d", data->state, data->point.x, data->point.y);
#endif
