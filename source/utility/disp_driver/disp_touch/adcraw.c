...
#if CONFIG_DISP_TOUCH_CONTROLLER_ADCRAW
...
static void setup_axis(gpio_num_t plus, gpio_num_t minus, gpio_num_t measure, gpio_num_t ignore)
{
	...
	GB_GPIO_Reset(ignore);
	GB_GPIO_SetDirection(ignore, GPIO_MODE_DISABLE);
	gpio_set_pull_mode(ignore, GPIO_FLOATING);
	GB_GPIO_Reset(measure);
	GB_GPIO_SetDirection(measure, GPIO_MODE_DISABLE);
...
#endif //CONFIG_DISP_TOUCH_CONTROLLER_ADCRAW
