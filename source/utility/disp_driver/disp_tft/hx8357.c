...
void hx8357_init(void)
{
	//Initialize non-SPI GPIOs
        GB_GPIO_Reset(HX8357_DC);
	GB_GPIO_SetDirection(HX8357_DC, GB_GPIO_OUTPUT);
        GB_GPIO_Reset(HX8357_RST);
	GB_GPIO_SetDirection(HX8357_RST, GB_GPIO_OUTPUT);

#if HX8357_ENABLE_BACKLIGHT_CONTROL
        GB_GPIO_Reset(HX8357_BCKL);
	GB_GPIO_SetDirection(HX8357_BCKL, GB_GPIO_OUTPUT);
#endif

	//Reset the display
	GB_GPIO_Set(HX8357_RST, 0);
	vTaskDelay(10 / portTICK_RATE_MS);
	GB_GPIO_Set(HX8357_RST, 1);
...
static void hx8357_send_cmd(uint8_t cmd)
{
	...
	GB_GPIO_Set(HX8357_DC, 0);	 /*Command mode*/
...
{
	disp_wait_for_pending_transactions();
	GB_GPIO_Set(HX8357_DC, 1);	 /*Data mode*/
