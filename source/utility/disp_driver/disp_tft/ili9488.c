...
void ili9488_init(void)
{
	...
	//Initialize non-SPI GPIOs
        GB_GPIO_Reset(ILI9488_DC);
	GB_GPIO_SetDirection(ILI9488_DC, GB_GPIO_OUTPUT);
        GB_GPIO_Reset(ILI9488_RST);
	GB_GPIO_SetDirection(ILI9488_RST, GB_GPIO_OUTPUT);

#if ILI9488_ENABLE_BACKLIGHT_CONTROL
        GB_GPIO_Reset(ILI9488_BCKL);
	GB_GPIO_SetDirection(ILI9488_BCKL, GB_GPIO_OUTPUT);
#endif

	//Reset the display
	GB_GPIO_Set(ILI9488_RST, 0);
	vTaskDelay(100 / portTICK_RATE_MS);
	GB_GPIO_Set(ILI9488_RST, 1);
	vTaskDelay(100 / portTICK_RATE_MS);

	GB_DEBUGI(DISP_TAG, "ILI9488 initialization.");
	...
	ili9488_enable_backlight(true);
        ...
        ili9488_set_orientation(CONFIG_DISPLAY_ORIENTATION);
}
...
static void ili9488_send_cmd(uint8_t cmd)
{
    ...
    GB_GPIO_Set(ILI9488_DC, 0);	 /*Command mode*/
...
{
    disp_wait_for_pending_transactions();
    GB_GPIO_Set(ILI9488_DC, 1);	 /*Data mode*/
