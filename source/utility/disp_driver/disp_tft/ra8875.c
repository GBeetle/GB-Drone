...
#if ( CONFIG_DISPLAY_ORIENTATION_PORTRAIT_INVERTED || CONFIG_DISPLAY_ORIENTATION_LANDSCAPE_INVERTED )
    #if CONFIG_INVERT_DISPLAY
...
#if CONFIG_DISP_RA8875_PCLK_INVERT
    #define PCSR_VAL (0x80 | CONFIG_DISP_RA8875_PCLK_MULTIPLIER)
#else
    #define PCSR_VAL (CONFIG_DISP_RA8875_PCLK_MULTIPLIER)
#endif

// Calculate horizontal display parameters
#if (CONFIG_DISP_RA8875_HORI_NON_DISP_PERIOD >= 260)
    #define HNDR_VAL (31)
#else
    #define HNDR_VAL ((CONFIG_DISP_RA8875_HORI_NON_DISP_PERIOD-12) / 8)
#endif
#define HNDFT (CONFIG_DISP_RA8875_HORI_NON_DISP_PERIOD-(8*HNDR_VAL)-12)
#if LVGL_DISP_RA8875_DE_POLARITY
    #define HNDFTR_VAL (0x80 | HNDFT)
#else
    #define HNDFTR_VAL (HNDFT)
#endif
#define HSTR_VAL (CONFIG_DISP_RA8875_HSYNC_START/8 - 1)
#define HPW (CONFIG_DISP_RA8875_HSYNC_PW/8 - 1)
...
#define VNDR_VAL (CONFIG_DISP_RA8875_VERT_NON_DISP_PERIOD - 1)
#define VSTR_VAL (CONFIG_DISP_RA8875_VSYNC_START - 1)
#define VPW (CONFIG_DISP_RA8875_VSYNC_PW - 1)
...
void ra8875_init(void)
{
    ...
    GB_DEBUGI(DISP_TAG, "Initializing RA8875...");

#if (CONFIG_DISP_PIN_BCKL == 15)
    ...
    GB_GPIO_Reset(RA8875_RST);
    GB_GPIO_SetDirection(RA8875_RST, GB_GPIO_OUTPUT);
#ifdef CONFIG_DISP_PIN_BCKL
    GB_GPIO_Reset(CONFIG_DISP_PIN_BCKL);
    GB_GPIO_SetDirection(CONFIG_DISP_PIN_BCKL, GB_GPIO_OUTPUT);
#endif

    // Reset the RA8875
    GB_GPIO_Set(RA8875_RST, 0);
    vTaskDelay(DIV_ROUND_UP(100, portTICK_RATE_MS));
    GB_GPIO_Set(RA8875_RST, 1);
    vTaskDelay(DIV_ROUND_UP(100, portTICK_RATE_MS));
...
{
#if CONFIG_ENABLE_BACKLIGHT_CONTROL
...
#ifdef CONFIG_DISP_PIN_BCKL
    GB_GPIO_Set(CONFIG_DISP_PIN_BCKL, tmp);
#endif
...
{
    GB_DEBUGI(DISP_TAG, "%s display.", enable ? "Enabling" : "Disabling");
...
#if DEBUG
    GB_DEBUGI(DISP_TAG, "flush: %d,%d at %d,%d", area->x1, area->x2, area->y1, area->y2 );
#endif
...
#if DEBUG
        GB_DEBUGI(DISP_TAG, "flush: set window (x1,x2): %d,%d -> %d,%d", x1, x2, area->x1, area->x2);
#endif
...
#if DEBUG
        GB_DEBUGI(DISP_TAG, "flush: set cursor (x,y): %d,%d -> %d,%d", x, y, area->x1, area->y1);
#endif
...
void ra8875_configure_clocks(bool high_speed)
{
    ...
    val = high_speed ? ((CONFIG_DISP_RA8875_PLLDIVM << 7) | CONFIG_DISP_RA8875_PLLDIVN) : 0x07;
    ra8875_write_cmd(RA8875_REG_PLLC1, val);           // PLL Control Register 1 (PLLC1)
    vTaskDelay(1);

    val = high_speed ? CONFIG_DISP_RA8875_PLLDIVK : 0x03;
