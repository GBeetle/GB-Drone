...
void ili9486_init(void)
{
	...
	//Initialize non-SPI GPIOs
        GB_GPIO_Reset(ILI9486_DC);
	GB_GPIO_SetDirection(ILI9486_DC, GB_GPIO_OUTPUT);
        GB_GPIO_Reset(ILI9486_RST);
	GB_GPIO_SetDirection(ILI9486_RST, GB_GPIO_OUTPUT);

#if ILI9486_ENABLE_BACKLIGHT_CONTROL
    GB_GPIO_Reset(ILI9486_BCKL);
    GB_GPIO_SetDirection(ILI9486_BCKL, GB_GPIO_OUTPUT);
#endif

	//Reset the display
	GB_GPIO_Set(ILI9486_RST, 0);
	vTaskDelay(100 / portTICK_RATE_MS);
	GB_GPIO_Set(ILI9486_RST, 1);
	vTaskDelay(100 / portTICK_RATE_MS);

	GB_DEBUGI(DISP_TAG, "ILI9486 Initialization.");
	...
	ili9486_enable_backlight(true);
        ...
        ili9486_set_orientation(CONFIG_DISPLAY_ORIENTATION);
}
...
static void ili9486_send_cmd(uint8_t cmd)
{
	...
	GB_GPIO_Set(ILI9486_DC, 0);	 /*Command mode*/
...
static void ili9486_send_data(void * data, uint16_t length)
{
	...
	disp_wait_for_pending_transactions();
	GB_GPIO_Set(ILI9486_DC, 1);	 /*Data mode*/
...
#if defined (CONFIG_PREDEFINED_DISPLAY_NONE)
