...
void sh1107_init(void)
{
...
#if defined CONFIG_INVERT_COLORS
	...
	//Initialize non-SPI GPIOs
        GB_GPIO_Reset(SH1107_DC);
	GB_GPIO_SetDirection(SH1107_DC, GB_GPIO_OUTPUT);
        GB_GPIO_Reset(SH1107_RST);
	GB_GPIO_SetDirection(SH1107_RST, GB_GPIO_OUTPUT);

	//Reset the display
	GB_GPIO_Set(SH1107_RST, 0);
	vTaskDelay(100 / portTICK_RATE_MS);
	GB_GPIO_Set(SH1107_RST, 1);
	vTaskDelay(100 / portTICK_RATE_MS);
...
{
	/* buf_w will be ignored, the configured CONFIG_DISPLAY_HEIGHT and _WIDTH,
	   and CONFIG_DISPLAY_ORIENTATION_LANDSCAPE and _PORTRAIT will be used. */
...
static void sh1107_send_cmd(uint8_t cmd)
{
    ...
    GB_GPIO_Set(SH1107_DC, 0);	 /*Command mode*/
...
{
    disp_wait_for_pending_transactions();
    GB_GPIO_Set(SH1107_DC, 1);	 /*Data mode*/
