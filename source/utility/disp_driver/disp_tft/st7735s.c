...
{
#ifdef CONFIG_M5STICKC_HANDLE_AXP192
    i2c_master_init();
    axp192_init();
#endif
	...
	//Initialize non-SPI GPIOs
        GB_GPIO_Reset(ST7735S_DC);
	GB_GPIO_SetDirection(ST7735S_DC, GB_GPIO_OUTPUT);
        GB_GPIO_Reset(ST7735S_RST);
	GB_GPIO_SetDirection(ST7735S_RST, GB_GPIO_OUTPUT);

	//Reset the display
	GB_GPIO_Set(ST7735S_RST, 0);
	vTaskDelay(100 / portTICK_RATE_MS);
	GB_GPIO_Set(ST7735S_RST, 1);
	vTaskDelay(100 / portTICK_RATE_MS);

	GB_DEBUGI(DISP_TAG, "ST7735S initialization.");
    ...
    st7735s_set_orientation(CONFIG_DISPLAY_ORIENTATION);
}
...
static void st7735s_send_cmd(uint8_t cmd)
{
	...
	GB_GPIO_Set(ST7735S_DC, 0);	 /*Command mode*/
...
{
	disp_wait_for_pending_transactions();
	GB_GPIO_Set(ST7735S_DC, 1);	 /*Data mode*/
...
static void axp192_init()
{
	...
	axp192_sleep_out();
	GB_DEBUGI(DISP_TAG, "AXP192 initialized, power enabled for LDO2 and LDO3");
}
