...
void st7796s_init(void)
{
	...
	GB_GPIO_Reset(ST7796S_DC);
	GB_GPIO_SetDirection(ST7796S_DC, GB_GPIO_OUTPUT);
	GB_GPIO_Reset(ST7796S_RST);
	GB_GPIO_SetDirection(ST7796S_RST, GB_GPIO_OUTPUT);

#if ST7796S_ENABLE_BACKLIGHT_CONTROL
	GB_GPIO_Reset(ST7796S_BCKL);
	GB_GPIO_SetDirection(ST7796S_BCKL, GB_GPIO_OUTPUT);
#endif
	//Reset the display
	GB_GPIO_Set(ST7796S_RST, 0);
	vTaskDelay(100 / portTICK_RATE_MS);
	GB_GPIO_Set(ST7796S_RST, 1);
	vTaskDelay(100 / portTICK_RATE_MS);
	...
	st7796s_set_orientation(CONFIG_DISPLAY_ORIENTATION);
...
static void st7796s_send_cmd(uint8_t cmd)
{
	...
	GB_GPIO_Set(ST7796S_DC, 0); /*Command mode*/
...
{
	disp_wait_for_pending_transactions();
	GB_GPIO_Set(ST7796S_DC, 1); /*Data mode*/
...
#elif defined(CONFIG_PREDEFINED_DISPLAY_WT32_SC01)
