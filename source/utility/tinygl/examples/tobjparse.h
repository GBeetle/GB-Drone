    ...
    {                                               \
        ...
        while (fgets(line, 2047, f)) {
            ...
            if (line[0] == 'v' && line[1] == 'c' && (strlen(line) > 4)) {
                // read=sscanf(line,"vc %f %f
                // %f",&val.d[0],&val.d[1],&val.d[2]); sb_push(retval.colors,
                // val);
            ...
            if (line[0] == 'v' && line[1] == 'n' && (strlen(line) > 4)) {
                // read=sscanf(line,"vn %f %f
                // %f",&val.d[0],&val.d[1],&val.d[2]); printf("\nn Read:
                // %d",read); sb_push(retval.normals, val);
            ...
            if (line[0] == 'f' && (strlen(line) > 4)) {
                ...
                // Grab the texture coordinates (First character after first
                // slash!)
                if (retval.texcoords) {
                    char *t = line + 2;
                    while (!isspace((unsigned char) *t) && *t != '\0' &&
